...
from api_test.serializers import PersonSerializer,MemberSerializer
...
def view_member(request):
    # GET method to retrieve all persons
    if request.method == 'GET':
        person_obj = Member.objects.all()
        serializer = MemberSerializer(person_obj, many=True)
        return Response({'msg': 'Successfully retrieved data', 'data': serializer.data}, status=status.HTTP_200_OK)
    ...
    # POST method to create a new person
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'msg': 'Member created successfully', 'data': serializer.data}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
